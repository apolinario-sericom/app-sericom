...
def main(page: ft.Page):
    ...
    def on_upload_result(e: ft.FilePickerResultEvent):
        ...
        try:
            ...
            page.snack_bar = ft.SnackBar(ft.Text(f"Erro ao enviar. No PC web trava, tente no app nativo depois!")); page.snack_bar.open = True; page.update()
    ...
    def mostrar_tela_aluno(aluno_dados):
        ...
        try:
            ...
            # --- 1. ABA DE DESAFIOS & FÁBRICA DE QUIZ ---
                ...
                if res_campanhas.data:
                    for c in res_campanhas.data:
                        try:
                            ...
                            if is_admin or c.get('publico_alvo', 'Todos') in ["Todos", aluno_dados.get('turma')]:
            ...
            if is_admin:
                ...
                try:
                    ...
                    turmas_bd = list(set([t.get('turma') for t in res_todas_t.data if t.get('turma')])) if res_todas_t.data else []
                    turmas_quiz.extend(turmas_bd)
                except: pass
                ...
                dd_camp_turma = ft.Dropdown(label="Público Alvo", options=[ft.dropdown.Option(t) for t in turmas_quiz], value="Todos")
                ...
                def preencher_camp(camp):
                    estado_quiz_admin["editando_id"] = camp['id']; campo_camp_tit.value = camp['titulo']; campo_camp_data.value = camp['data_limite']; dd_camp_turma.value = camp['publico_alvo']
                    btn_salvar_camp.text = "💾 ATUALIZAR CAMPANHA"; btn_salvar_camp.bgcolor = ft.Colors.ORANGE_600; page.update()

                def salvar_nova_campanha(e):
                    if not campo_camp_tit.value: return
                    try:
                        if estado_quiz_admin["editando_id"]:
                            supabase.table("quiz_campanhas").update({"titulo": campo_camp_tit.value, "data_limite": campo_camp_data.value, "publico_alvo": dd_camp_turma.value}).eq("id", estado_quiz_admin["editando_id"]).execute()
                            ...
                            supabase.table("quiz_campanhas").insert({"titulo": campo_camp_tit.value, "data_limite": campo_camp_data.value, "publico_alvo": dd_camp_turma.value}).execute(); page.snack_bar = ft.SnackBar(ft.Text("✅ Campanha Criada!"))
                        campo_camp_tit.value = ""; carregar_campanhas_admin(); page.snack_bar.open = True; page.update()
            ...
            # --- 2. ABAS DE RANKING, PROGRESSO E FALTAS ---
            ...
            # --- 5. MURAL SOCIAL (AGORA COM PERMISSÃO DE EDIÇÃO PARA O DONO) ---
                ...
                try:
                    ...
                    if res_posts.data:
                        for p in res_posts.data:
                            ...
                            # A MÁGICA DE PERMISSÃO ACONTECE AQUI! 
                            sou_o_dono = p.get('autor') == aluno_dados.get('nome')
                            pode_editar = is_admin or sou_o_dono
            ...
            # --- 6. ABA GALERIA DE OURO (SÓ ADMIN POSTA) ---
            campo_autor_galeria = ft.TextField(label="Nome do Artista (Aluno)", border_color=ft.Colors.AMBER_400)
            campo_desc_galeria = ft.TextField(label="Descrição (Ex: Panfleto CorelDRAW)", border_color=ft.Colors.AMBER_400)
            ...
            def postar_galeria(e):
                if not campo_autor_galeria.value or not campo_img_galeria.value: page.snack_bar = ft.SnackBar(ft.Text("Faltou o nome ou a foto!")); page.snack_bar.open = True; page.update(); return
                try:
                ...
                except Exception as ex: print("Erro Galeria:", ex)
            ...
            def criar_tela_galeria():
                ...
                try:
                    ...
                    else: lista_gal.controls.append(ft.Text("O Museu ainda está vazio.", text_align="center", color=ft.Colors.GREY_500))
                except: pass
            ...
            # --- LÓGICA DO SININHO DE NOTIFICAÇÕES IN-APP ---
            def checar_notificacoes(e):
                mensagens_notificacao = []
                agora_n = datetime.now()
                
                # 1. Checa Desafios Disponíveis
                if desafios_disponiveis and not is_admin:
                    mensagens_notificacao.append(ft.Container(padding=10, bgcolor=ft.Colors.GREY_900, border_radius=5, content=ft.Row([ft.Icon(ft.Icons.GAMES, color=ft.Colors.GREEN_400), ft.Text("Você tem Desafios valendo pontos!", color=ft.Colors.WHITE, expand=True)])))
                
                # 2. Checa Mensalidades
                if not is_admin:
                    try:
            ...
            page.add(ft.Column([
                ft.Container(bgcolor=ft.Colors.RED_900, padding=15, content=ft.Row([
                    ft.GestureDetector(content=foto_topo, on_tap=abrir_carteirinha), 
                    ft.Text(f" {aluno_dados.get('nome','')}", size=16, weight="bold", color=ft.Colors.WHITE, expand=True), 
                    ft.IconButton(ft.Icons.NOTIFICATIONS_ACTIVE, icon_color=ft.Colors.AMBER_400, on_click=checar_notificacoes), # O SINO AQUI!
            ...
            page.add(ft.Container(padding=50, content=ft.Column([ft.Icon(ft.Icons.ERROR, color="red", size=50), ft.Text("Oops! Algo deu errado ao carregar seus dados.", size=20, weight="bold"), ft.Text(str(erro_master), color="red"), ft.ElevatedButton("Voltar para Login", on_click=lambda _: mostrar_tela_login())]))); page.update()
    ...
    def fazer_login(e):
        ...
        except Exception as ex: print("Erro login:", ex); texto_erro.value = "Erro de conexão!"; page.update()
