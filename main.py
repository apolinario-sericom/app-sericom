                ...
                if is_admin:
                    ...
                    def adicionar_pergunta_temp(e):
                        if not dd_perg_campanha.value or not campo_perg_txt.value or not campo_opt_a.value or not dd_certa.value:
                            page.snack_bar = ft.SnackBar(ft.Text("Preencha os campos da pergunta!"))
                        ...
                        campo_perg_txt.value = ""
                        campo_perg_img.value = ""
                        campo_opt_a.value = ""
                        campo_opt_b.value = ""
                        campo_opt_c.value = ""
                        campo_opt_d.value = ""
                        atualizar_lista_temp()
                        page.snack_bar = ft.SnackBar(ft.Text("✅ Pergunta guardada na lista!"))
                        page.snack_bar.open = True
                        page.update()

                    def lancar_quiz(e):
                        if not perguntas_temp:
                            page.snack_bar = ft.SnackBar(ft.Text("Adicione pelo menos uma pergunta antes de lançar!"))
                            page.snack_bar.open = True
                            page.update()
                            return
                        try:
                            ...
                            atualizar_lista_temp()
                            page.snack_bar = ft.SnackBar(ft.Text("🚀 Quiz Lançado com Sucesso no App!"), bgcolor=ft.Colors.GREEN_600)
                            page.snack_bar.open = True
                            page.update()
                        except Exception as ex:
                            page.snack_bar = ft.SnackBar(ft.Text(f"Erro ao lançar: {ex}"))
                            page.snack_bar.open = True
                            page.update()
